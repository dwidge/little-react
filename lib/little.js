...
const debugRender = () => {
  ...
  if (debugOverlay) {
    ...
    for (const o of engineObjects) {
      if (o.canvas || o.destroyed) continue; // skip tile layers
 ...
 *  @memberof Utilities */
const wave = (frequency = 1, amplitude = 1, t = time) =>
  (amplitude / 2) * (1 - Math.cos(t * frequency * 2 * PI));
 ...
 *  @default
 *  @memberof Settings */
let objectDefaultDamping = 0.99;

/** How much to slow angular velocity each frame (0-1)
 *  @default
 *  @memberof Settings */
let objectDefaultAngleDamping = 0.99;
...
const keyIsDown = (key, device = 0) =>
  inputData[device] && inputData[device][key] & 1 ? 1 : 0;
...
const keyWasPressed = (key, device = 0) =>
  inputData[device] && inputData[device][key] & 2 ? 1 : 0;
...
const keyWasReleased = (key, device = 0) =>
  inputData[device] && inputData[device][key] & 4 ? 1 : 0;
...
const gamepadWasPressed = (button, gamepad = 0) =>
  keyWasPressed(button, gamepad + 1);
...
const gamepadWasReleased = (button, gamepad = 0) =>
  keyWasReleased(button, gamepad + 1);
...
const gamepadStick = (stick, gamepad = 0) =>
  stickData[gamepad] ? stickData[gamepad][stick] || vec2() : vec2();
  ...
  for (let i = gamepads.length; i--; ) {
    ...
    if (gamepad) {
      ...
      const applyDeadZone = (v) =>
        v > deadZone
          ? percent(v, deadZone, deadZoneMax)
          : v < -deadZone
          ? -percent(-v, deadZone, deadZoneMax)
          : 0;
      ...
      for (let j = 0; j < gamepad.axes.length - 1; j += 2)
        sticks[j >> 1] = vec2(
          applyDeadZone(gamepad.axes[j]),
          applyDeadZone(-gamepad.axes[j + 1])
        ).clampLength();

      // read buttons
      for (let j = gamepad.buttons.length; j--; ) {
        const button = gamepad.buttons[j];
        data[j] = button.pressed
          ? 1 + 2 * !gamepadIsDown(j, i)
          : 4 * gamepadIsDown(j, i);
  ...
  ontouchstart =
    ...
    ontouchend =
      (e) => {
        ...
        } else if (wasTouching) onmouseup(e);
...
function touchGamepadCreate() {
  if (!touchGamepadEnable || !isTouchDevice) return;
        ...
        for (const touch of e.touches) {
          ...
          if (touchPos.distance(stickCenter) < touchGamepadSize) {
            ...
            if (touchGamepadAnalog)
              ...
              touchGamepadStick.setAngle(
                ((((angle * 4) / PI + 8.5) | 0) * PI) / 4
              );
            }
...
function touchGamepadRender() {
  ...
  for (let i = 4; i--; ) {
    const pos = rightCenter.add(
      new Vector2().setAngle((i * PI) / 2, touchGamepadSize / 2)
    );
 ...
 *  @memberof Audio */
const getNoteFrequency = (semitoneOffset, rootFrequency = 220) =>
  rootFrequency * 2 ** (semitoneOffset / 12);
...
function zzfxG(
  ...
  let PI2 = PI * 2,
    ...
    startFrequency = (frequency *=
      ((1 + randomness * rand(-1, 1)) * PI2) / zzfxR),
    t = 0,
    tm = 0,
    i = 0,
    j = 1,
...
/** Set tile collision
 *  @memberof TileCollision */
const setTileCollision = (data) => {
  if (!(tileCollision.length === tileCollisionSize.area()))
    throw new Error("setTileCollisionE1");
  tileCollision = data;
};
...
class TileLayer extends EngineObject {
  ...
  redrawEnd() {
    ...
    [mainCanvas, mainContext, cameraPos, cameraScale] =
      this.savedRenderSettings;
  }
...
class Particle extends EngineObject {
  ...
  render() {
    ...
    if (this.trailScale) {
      ...
      drawTile(
        this.pos.add(direction.multiply(vec2(0, -trailLength / 2))),
        size,
        this.tileIndex,
        this.tileSize,
        color,
...
function glInit(div) {
  ...
  glOverlay && div.appendChild(glCanvas);
...
function glCreateTexture(image) {
  ...
  glContext.texParameteri(
    ...
    cavasPixelated ? gl_NEAREST : gl_LINEAR
  );
  glContext.texParameteri(
    gl_TEXTURE_2D,
...
function glFlush() {
  ...
  glContext.bufferSubData(
    ...
    glPositionData.subarray(
      0,
      glBatchCount * gl_VERTICES_PER_QUAD * gl_INDICIES_PER_VERT
    )
  );
...
function engineInit(
  ...
  tileImage.onerror = tileImage.onload = () => {
    ...
    tileImageFixBleed = vec2(tileFixBleedScale).divide(
    ...
    div.appendChild((mainCanvas = document.createElement("canvas")));
    ...
    div.appendChild((overlayCanvas = document.createElement("canvas")));
  ...
  const engineUpdate = (frameTimeMS = 0) => {
    ...
    if (debug || showWatermark)
      averageFPS = lerp(0.05, averageFPS || 0, 1e3 / (frameTimeDeltaMS || 1));
    if (debug)
      frameTimeDeltaMS *= keyIsDown(107) ? 5 : keyIsDown(109) ? 0.2 : 1; // +/- to speed/slow time
    ...
    if (canvasFixedSize.x) {
      ...
      overlayCanvas.width = mainCanvas.width = min(
        div.clientWidth,
        canvasMaxSize.x
      );
      overlayCanvas.height = mainCanvas.height = min(
        div.clientHeight,
        canvasMaxSize.y
      );
    }
